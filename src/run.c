...
int main(void) {
	// Load model
	OEC_MODEL *model = oec_model_load("/sdcard/one-eyed-cobra/best.oec");
	...
	if (oec_image_load("/sdcard/one-eyed-cobra/dataset/test/images/img_00033.png", &image) != 0) {
	...
	if (oec_image_save_png("/sdcard/one-eyed-cobra/prediction/predict_00033.png", &image) != 0) {
		fprintf(stderr, "Failed to save test image\n");
		oec_image_free(&image);
		oec_model_free(model);
		return EXIT_FAILURE;
	}
